/**
 *Submitted for verification at hecoinfo.com on 2021-07-03
*/
...
contract MasterChef is Ownable {
    ...
    event UpdateEmissionRate(address indexed user, uint256 _dinoPerBlock);
    ...
    function pendingdino(uint256 _pid, address _user) external view validatePoolByPid(_pid) returns (uint256) {
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 dinoReward = multiplier.mul(dinoPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        uint256 dinoRewardToDev = dinoReward.div(10);
        dino.mint(devaddr, dinoRewardToDev);
        dino.mint(address(this), dinoReward.sub(dinoRewardToDev));
...
contract DinoYield is HRC20('DinoDefi', 'Dino') {
