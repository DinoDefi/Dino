...
contract MasterChef is Ownable {
    ...
    struct PoolInfo {
        ...
        uint256 accdytPerShare;
        uint16 depositFeeBP;
    }


    DinoYield public dyt;
    address public devaddr;
    uint256 public dytPerBlock;
    ...
    function pendingdyt(uint256 _pid, address _user) external view validatePoolByPid(_pid) returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            accdytPerShare = accdytPerShare.add(dytReward.mul(1e12).div(lpSupply));
        }
        return user.amount.mul(accdytPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public validatePoolByPid(_pid) {
        ...
        updatePool(_pid);
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accdytPerShare).div(1e12).sub(user.rewardDebt);
            if(pending > 0) {
                safedytTransfer(msg.sender, pending);
            }
        }
        if(_amount > 0) {
        ...
        user.rewardDebt = user.amount.mul(pool.accdytPerShare).div(1e12);
    ...
    function safedytTransfer(address _to, uint256 _amount) internal {
        uint256 dytBal = dyt.balanceOf(address(this));
        if (_amount > dytBal) {
            dyt.transfer(_to, dytBal);
        } else {
            dyt.transfer(_to, _amount);
        }
    }
...
contract DinoYield is HRC20('Dino Yield Token', 'DYT') {
        ...
        external
    {
        ...
        require(signatory != address(0), "dyt::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "dyt::delegateBySig: invalid nonce");
        require(now <= expiry, "dyt::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "dyt::getPriorVotes: not yet determined");
        ...
        internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying dyts (not scaled);
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "dyt::_writeCheckpoint: block number exceeds 32 bits");
